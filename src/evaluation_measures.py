...
def psds_score(psds, filename_roc_curves=None):
    ...
    Args:
        psds: psds.PSDSEval object list initialized with the groundtruth corresponding to the predictions
        filename_roc_curves: str, the base filename of the roc curve to be computed
    """
    try:
        psds_0 = psds[0].psds(alpha_ct=0, alpha_st=0, max_efpr=100)
        logger.info(f"\nPSD-Score dcase 20 default: {psds_0.value:.5f}")
        psds_1 = psds[1].psds(alpha_ct=0, alpha_st=1, max_efpr=100)
        logger.info(f"\nPSD-Score scenario 1: {psds_1.value:.5f}")
        psds_2 = psds[2].psds(alpha_ct=0.5, alpha_st=1, max_efpr=100)
        logger.info(f"\nPSD-Score scenario 2: {psds_2.value:.5f}")
        ...
        if filename_roc_curves is not None:
            ...
            plot_psd_roc(psds_0, filename=f"{base}_0{ext}")
            plot_psd_roc(psds_1, filename=f"{base}_1{ext}")
            plot_psd_roc(psds_2, filename=f"{base}_2{ext}")
