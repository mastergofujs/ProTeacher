import torch
from models.conformer.conformer_encoder import PromptedConformerMaskedEncoder
...
class SEDModel(torch.nn.Module):
    ...
    def forward(self, input, prompt_tuning=True):
        ...
        if self.pooling == "token":
            ...
            cls_token = False
4333m 222222222222222222222222222222222        x, _ = self.encoder_masked(x, inds=(masked_inds, unmasked_inds), cls_token=cls_token, prompt_tuning=prompt_tuning)
        x_m, _ = self.encoder_masked(x_m, inds=(masked_inds, unmasked_inds), 
                                     unmasked_only=True, cls_token=cls_token,
                                     prompt_tuning=False)
        
        dec = self.decoder(x_m, inds=(masked_inds, unmasked_inds), prompt_nums=self.encoder_masked.pnums, cls_token=cls_token)
