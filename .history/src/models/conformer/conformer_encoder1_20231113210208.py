...
class PromptedConformerMaskedEncoder(torch.nn.Module):
    ...
    def forward(self, x, inds, mask=None, cls_token=True, unmasked_only=False, prompt_tuning=True):
        ...
        if unmasked_only:
            if cls_token:
                inds_all = np.concatenate([torch.zeros(1, dtype=torch.int64), unmasked_id + self.pnums + 1])
            else:
                inds_all = unmasked_id + self.pnums + 1
        else:
            if prompt_tuning:
                if cls_token:
                    x = torch.cat([x[:, 0:1, :], 
                ...
                inds_all = np.concatenate([torch.zeros(1, dtype=torch.int64), 
