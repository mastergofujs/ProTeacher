...
class PromptedConformerMaskedEncoder(torch.nn.Module):
    ...
    def forward(self, x, inds, cls_token=True, unmasked_only=False, prompt_tuning=True):
        masked_id, unmasked_id = inds[0], inds[1]
        if cls_token:
        ...
        if not unmasked_only:
            if prompt_tuning:
                ...
                x_u = x
                inds_all = torch.arange(self.prompt_tokens.size(1), self.prompt_tokens.size(1) + fn + 1)
                inds_all[0] = 0
        else: 
            if cls_token:
                unmasked_ = torch.cat([torch.zeros(1, dtype=torch.int64), unmasked_id + 1])
