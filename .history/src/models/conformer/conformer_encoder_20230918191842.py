import torch
from src.models.conformer.conformer_block import ConformerBlock
from src.models.conformer.repeat import repeat
from src.models.transformer.embedding import PositionalEncoding
from src.models.conformer.masked_positional_encoding import MaskedPositionalEncoding
...
class ConformerPromptedEncoder(torch.nn.Module):
    def __init__(
        ...
        self.prompt_tokens = PromptEmbedding(adim, prompt_nums, self.player, deep=True)._get_prompt_embedding()
        
    def forward(self, x, mask=None, prompt_tuning=True):
        # mask defination
        mask = torch.zeros(x.size(1) + self.prompt_tokens.size(1), 
                           x.size(1) + self.prompt_tokens.size(1), 
        ...
        for e in range(self.elayer):
            ...
            if e < self.player:
                if e ==0:
                    x = torch.cat([x[:, 0:1, :], self.prompt_tokens[e, :, :].expand(x.size(0), -1, -1), x[:, 1:, :]], dim=1)
                else:
                    x[:, 1: 1 + self.prompt_tokens.size(1), :] = self.prompt_tokens[e, :, :].expand(x.size(0), -1, -1)
        
            if not prompt_tuning:
                x, mask = self.conformer_blocks[e](x, mask)
            else:
                x, _ = self.conformer_blocks[e](x, None)
