...
class ConformerPromptedEncoder(torch.nn.Module):
    ...
    def forward(self, x, prompt_tuning, mask=None):
        ...
        for e in range(self.elayer):
            ...
            if e < self.player:
                if e ==0:
                    x = torch.cat([x[:, 0:1, :], self.input_layer(self.prompt_tokens[e, :, :].expand(x.size(0), -1, -1)), 
                                   x[:, 1:, :]], dim=1)
                else:
                    x[:, 1: 1 + self.prompt_tokens.size(1), :] = self.input_layer(
                        self.prompt_tokens[e, :, :].expand(x.size(0), -1, -1))
