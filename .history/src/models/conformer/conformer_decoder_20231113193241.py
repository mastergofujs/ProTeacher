...
class ConformerMaskedDecoder(torch.nn.Module):
    ...
    def forward(self, x, inds, with_prompts=False, cls_token=True):
        masked_id, unmasked_id = inds[0], inds[1]
        b, nf, d = x.size()
        if cls_token:
            # Note: here input x is unmasked with cls_token
            if with_prompts:
                inds_all = torch.cat([torch.tensor(masked_id) + 1 + self.pnums, 
                                    torch.zeros(1, dtype=torch.int64), 
                                    torch.tensor(unmasked_id) + 1 + self.pnums]) 
            else:
                inds_all = torch.cat([torch.tensor(masked_id) + 1, 
                torch.zeros(1, dtype=torch.int64), 
                torch.tensor(unmasked_id) + 1]) 
        else:
            inds_all = torch.cat([torch.tensor(masked_id), torch.tensor(unmasked_id)])
        x_u = torch.zeros((b, len(inds_all), d)).cuda()
        if cls_token:
            unmasked_ = torch.cat([torch.zeros(1, dtype=torch.int64), torch.tensor(unmasked_id) + 1])
            x_u[:, torch.tensor(masked_id) + 1, :] = self.mask_embed
        else:
