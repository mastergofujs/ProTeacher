...
class ConformerMaskedDecoder0(torch.nn.Module):
    ...
    def forward(self, x, inds, prompt_nums=0, cls_token=True):
        masked_id, unmasked_id = inds[0], inds[1]
        b, nf, d = x.size()
        if cls_token:
            # Note: here input x is unmasked with cls_token
            inds_all = np.concatenate([masked_id + 1 + prompt_nums, 
                                        np.zeros(1, dtype=np.int64), 
                                        unmasked_id + 1 + prompt_nums])
...
class ConformerMaskedDecoder(torch.nn.Module):
    ...
    def forward(self, x, inds, prompt_nums=0, cls_token=True):
        ...
        if cls_token:
            ...
            unmasked_ = unmasked_id.sort()[0]
            x_u[:, masked_id.sort()[0], :] = self.mask_embed
        x_u[:, unmasked_, :] = x
        x_u = self.input_layer(x_u)
