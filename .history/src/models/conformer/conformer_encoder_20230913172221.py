...
class ConformerPromptedEncoder(torch.nn.Module):
    def __init__(
        ...
        self.prompt_tokens = PromptEmbedding(idim, prompt_nums, elayers, deep=True)._get_prompt_embedding()
    ...
    def forward(self, x, mask=None):
        ...
        for e in range(self.elayer):
            ...
            x = torch.cat([x[:, 0, :], self.prompt_tokens[:, :, e].expand(x.size(0), -1, -1), x[:, 1:, :]])
            if not self.tuning:
                x, mask = self.conformer_blocks[e](x, mask)
            else:
                x, _ = self.conformer_blockse[e](x, None)
