...
class ConformerMaskedEncoder(torch.nn.Module):
    ...
    def forward(self, x, inds, cls_token=True, unmasked_only=False):
        ...
        if not unmasked_only:
            ...
            if cls_token:
                unmasked_ = torch.cat([torch.zeros(1, dtype=torch.int64), torch.tensor(unmasked_id) + 1])
            else:
...
class PromptedConformerMaskedEncoder(torch.nn.Module):
    ...
    def forward(self, x, inds, mask=None, cls_token=True, unmasked_only=False, prompt_tuning=True):
        masked_id, unmasked_id = inds[0], inds[1]
        if cls_token:
            inds_all = torch.cat([torch.zeros(1, dtype=torch.int64), masked_id + 1 + self.pnums, unmasked_id + 1 + self.pnums])
        else:
            inds_all = torch.cat([masked_id + self.pnums, unmasked_id + self.pnums])
        b, fn, d = x.shape
        seq_len = x.size(1) - 1
        x_u = torch.zeros((b, len(inds_all), d)).cuda()
        if not unmasked_only:
            if prompt_tuning:
                x_u = torch.cat([x[:, 0:1, :], 
                                 ...
                                 x[:, 1:, :]],
                                dim=1)
                inds_all = torch.arange(0, x_u.size(1))
            else:
                x_u = x
                inds_all = torch.arange(self.prompt_tokens.size(1), self.prompt_tokens.size(1) + seq_len + 1)
                inds_all[0] = 0
        else: 
            if cls_token:
            ...
            x_u[:, unmasked_, :] = x
            x_u = x_u[:, unmasked_.sort()[0], :]
            inds_all = unmasked_.sort()[0] + self.prompt_tokens.size(1)
            inds_all[0] = 0
        x_u = self.input_layer(x_u)
        x_u = self.pe(x_u, inds_all)
        for e in range(self.elayer):
            ...
            if prompt_tuning:
                if e < self.player:
                    if e > 0 :
                        x_u[:, 1: 1 + self.prompt_tokens.size(1), :] = self.fc(self.prompt_tokens[e, :, :]).expand(x_u.size(0), -1, -1)
            x_u, mask = self.conformer_blocks[e](x_u, mask)
        x_u = torch.cat([x_u[:, 0:1], x_u[:, - seq_len:]], dim=1)
        return x_u, inds