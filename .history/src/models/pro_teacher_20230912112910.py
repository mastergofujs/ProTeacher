...
class SEDModel(torch.nn.Module):
    def __init__(
        ...
        self.encoder = ConformerEncoder(prompt_dim=input_dim, **encoder_kwargs)
        ...
        if self.pooling == "attention":
            ...
            self.prompt_tokens = PromptEmbedding(input_dim, **prompt_kwargs)
    ...
    def forward(self, x, mask=None, prompt_tuning=False):
        ...
        if self.pooling == "token":
            if prompt_tuning:
                x = torch.cat([self.tag_token, self.dropout(self.prompt_tokens), x], dim=1)
            else:
