...
from models.sed_model import SEDModel as Baseline
from models.joint_former import SEDModel
from loc_vad import activity_detection
from post_processing import EmbedsDataset
...
def parse_args(args):
    ...
    parser.add_argument("--exp_name", default="20_joint_former", type=str, help="exp name used for the training")
    parser.add_argument("--exp_name_base", default="conformer_sed_20", type=str, help="exp name used for the training")
    
    parser.add_argument("--debugmode", default=True, action="store_true", help="Debugmode")
    parser.add_argument("--verbose", "-V", default=0, type=int, help="Verbose option")
    parser.add_argument("--test_meta", default="/data0/gaolj/sed_data/DCASE2019/metadata/eval/public.tsv")
    parser.add_argument("--test_audio_dir", default="/data0/gaolj/sed_data/DCASE2019/audio/eval/public")
...
@torch.no_grad()
def test(exp_name, exp_name_base):
    h5_path = "{}/test/test_-posterior.h5".format(exp_name)
    h5_path_base = "{}/test/test_-posterior.h5".format(exp_name_base)
    
    data_loader = DataLoader(EmbedsDataset(h5_path , has_label=True))
    dataset_base = EmbedsDataset(h5_path_base , has_label=True)
    loc_figs_path = 'exp/sed_figs/dcase_{}/'.format(str(exp_name).split('/')[-1][:2])
    ...
    for batch_idx, data in enumerate(data_loader):
        ...
        axs[1].set_ylabel('ConformerSED', fontdict={'fontsize': 16, 'family': 'Times New Roman'})
        ...
        axs[2].set_ylabel('JointFormer', fontdict={'fontsize': 16, 'family': 'Times New Roman'})
...
def test_single(exp_name, exp_name_base, id):
    h5_path = "{}/test/test_-posterior.h5".format(exp_name)
    h5_path_base = "{}/test/test_-posterior.h5".format(exp_name_base)
    
    dataset = EmbedsDataset(h5_path , has_label=True)
    dataset_base = EmbedsDataset(h5_path_base , has_label=True)
    loc_figs_path = 'exp/sed_figs/dcase_{}/'.format(str(exp_name).split('/')[-1][:2])
    labels = ['Speech', 'Dog' , 'Cat', 'Alarm_bell_ringing', 'Dishes', 'Frying', 'Blender', 'Running_water', 'Vacuum_cleaner', 'Electric_shaver_toothbrush']
    event_labels = ['C1', 'C2', 'C3', 'C4', 'C5', 'C6', 'C7', 'C8', 'C9', 'C10']
    ...
    axs[1].set_xticks(np.arange(0, 60, 6))
    axs[1].set_xticklabels(np.arange(10.0), fontdict={'fontsize': 14, 'family': 'Times New Roman'})
    axs[1].imshow(extend_arr(smooth(sed_pred_base).T, 8), cmap='YlOrBr', aspect='auto')
    axs[1].set_yticks(np.arange(4, 10 * 8 + 4, 8))
    axs[1].set_yticklabels(event_labels, rotation=35, fontdict={'fontsize': 11, 'family': 'Times New Roman'})
    axs[1].set_ylabel('ConformerSED', fontdict={'fontsize': 16, 'family': 'Times New Roman'})
    axs[2].imshow(extend_arr((sed_pred).T, 8), cmap='YlOrBr', aspect='auto')
    axs[2].set_yticks(np.arange(4, 10 * 8 + 4, 8))
    axs[2].set_yticklabels(event_labels, rotation=35, fontdict={'fontsize': 11, 'family': 'Times New Roman'})
    axs[2].set_ylabel('LTE-Net', fontdict={'fontsize': 16, 'family': 'Times New Roman'})
...
def main(args):
    ...
    exp_name = Path(f"/home/gaolj/ConformerSED/exp/{args.exp_name}")
    exp_name_base = Path(f"/home/gaolj/ConformerSED/exp/{args.exp_name_base}")
    # test(exp_name, exp_name_base)
    test_single(exp_name, exp_name_base, id='zM515Ca0AiI_79_89.wav')
