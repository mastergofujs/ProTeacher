...
@torch.no_grad()
def test(exp_name):
    h5_path = "{}/test/syn-posterior.h5".format(exp_name)
    h5_path_base = "{}/test/syn-posterior-no-p.h5".format(exp_name)
    
    data_loader = DataLoader(ScoreDataset(h5_path , has_label=True))
    dataset_base = ScoreDataset(h5_path_base , has_label=True)
    loc_figs_path = 'exp/sed_figs/dcase_{}/syn/'.format(str(exp_name).split('/')[-1][:2])
...
def test_single(exp_name, exp_name_base, id):
    ...
    h5_path_base = "{}/test/test_-posterior-no-p.h5".format(exp_name_base)
