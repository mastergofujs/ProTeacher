...
class PostProcess:
    def __init__(
        ...
        self.get_posterior(save_h5_path=output_dir / "test-posterior.h5")
        self.data_loader = DataLoader(ScoreDataset(output_dir / "test-posterior.h5", has_label=True))
    ...
    def get_posterior(self, save_h5_path) -> None:
        with h5py.File(save_h5_path, "w") as h5:
            ...
            for (batch_input, batch_target, data_ids) in self.iterator:
                predicts = self.model(batch_input.to(self.device), prompt_tuning=False)
