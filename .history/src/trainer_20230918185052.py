...
class MeanTeacherTrainerOptions:
    ...
    patience: int = 10
    reconstruction_cost: float = 0.1,
    device = "cuda" if torch.cuda.is_available() else "cpu"
    withlogits: bool = True
    def _set_validation_options(
        ...
        self.classification_criterion_logits = torch.nn.BCEWithLogitsLoss().to(self.device)
        self.classification_criterion = torch.nn.BCELoss().to(self.device)
        self.consistency_criterion = torch.nn.MSELoss().to(self.device)
        self.recon_criterion = torch.nn.MSELoss().to(self.device)
        self.contrastive_criterion = InfoNCE(negative_mode='paired')
...
class MeanTeacherTrainer(object):
    def __init__(
        ...
        self.withlogits = trainer_options.withlogits
        ...
        if self.withlogits:
            self.classification_criterion = trainer_options.classification_criterion_logits
        else:    
        ...
        self.recon_criterion = trainer_options.recon_criterion
        self.contrastive_criterion = trainer_options.contrastive_criterion
        ...
        self.reconstruction_cost = trainer_options.reconstruction_cost
        ...
        self.contrastive_losses = AverageMeter()
        self.recon_losses = AverageMeter()
    ...
    def train_one_step(self) -> None:
        ...
        # Outputs
        with torch.no_grad():
            output_ema_s = self.ema_model(strong_sample, prompt_tuning=False)
            output_ema_w = self.ema_model(weak_sample, prompt_tuning=False)
            output_ema_u = self.ema_model(unlabel_sample, prompt_tuning=False)

        output_s = self.model(strong_sample, prompt_tuning=False)
        output_w = self.model(weak_sample, prompt_tuning=False)
        output_u = self.model(unlabel_sample, prompt_tuning=False)
        ...
        if 'recon_loss' in output_s:
            recon_loss = self.reconstruction_cost * (output_s["recon_loss"] + output_w["recon_loss"] + output_u["recon_loss"]) / 3
            # consistency_recon_weak = self.recon_criterion(output_w['recon'], output_ema_w['recon'])
            # consistency_recon_strong = self.recon_criterion(output_s['recon'], output_ema_s['recon'])
            # consistency_recon_unlabeled = self.recon_criterion(output_u['recon'], output_ema_u['recon'])
            # recon_loss_consistency = consistency_cost * (consistency_recon_weak + consistency_recon_strong + consistency_recon_unlabeled) / 3
            # recon_loss = recon_loss_self + recon_loss_consistency
        else:
            recon_loss = torch.zeros(1).cuda()

        if 'event' in output_s:
            # compute InfoNCE loss
            contrastive_loss = lambda query, p_key, n_keys: self.contrastive_criterion(query, p_key, n_keys)
            info_nce_w = torch.tensor(0.).cuda()
            info_nce_s = torch.tensor(0.).cuda()
            info_nce_u = torch.tensor(0.).cuda()
            b, k, _, _ = output_s["event"].size()
            for n in range(k):
                activated_b = weak_target[:, n] == 1
                if (activated_b == False).all():
                    continue
                query = output_w["event"][activated_b, n].mean(1)
                p_key = output_ema_w["event"][activated_b, n].mean(1)
                n_keys = torch.cat([output_ema_w["event"][activated_b, :n], output_ema_w["event"][activated_b, n + 1 :]], dim=1).mean(2)
                info_nce_w += contrastive_loss(query, p_key, n_keys)

            for n in range(k):
                activated_b = weak_target[:, n] == 1
                if (activated_b == False).all():
                    continue
                query = output_s["event"][activated_b, n].mean(1)
                p_key = output_ema_s["event"][activated_b, n].mean(1)
                n_keys = torch.cat([output_ema_s["event"][activated_b, :n], output_ema_s["event"][activated_b, n + 1 :]], dim=1).mean(2)
                info_nce_w += contrastive_loss(query, p_key, n_keys)

            for n in range(k):
                activated_b = output_ema_u["weak"][:, n].round() == 1
                if (activated_b == False).all():
                    continue
                query = output_u["event"][activated_b, n].mean(1)
                p_key = output_ema_u["event"][activated_b, n].mean(1)
                n_keys = torch.cat([output_ema_u["event"][activated_b, :n], output_ema_u["event"][activated_b, n + 1 :]], dim=1).mean(2)
                info_nce_w += contrastive_loss(query, p_key, n_keys)
            info_nce = 1.0 * (info_nce_w + info_nce_s + info_nce_u)
        else:
        ...
        self.recon_losses.update(recon_loss.item())
        self.contrastive_losses.update(info_nce.item())
        ...
        if self.forward_count % self.options.log_interval == 0:
            ...
            logging.info("\t contrastive loss weak: {}".format(info_nce.item()))
            logging.info("\t reconstruction loss: {}".format(recon_loss.item()))
            ...
            wandb.log(
                {
                    ...
                    "reconstruction_loss": self.recon_losses.avg,
                    "contrastive_loss": self.contrastive_losses.avg,
    ...
    def validation(self, post_processing=None) -> None:
        ...
        for (data, target, data_ids) in self.valid_loader:
            ...
            if self.withlogits:
                ...
                predicts["strong"] = predicts["strong"].cpu().data.numpy()
                predicts["weak"] = predicts["weak"].cpu().data.numpy()
