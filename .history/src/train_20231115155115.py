...
# from models.pado_net import PADOTransformer as SEDModel
...
def parse_args(args):
    ...
    parser.add_argument("--config", default="./config/dcase19_pado.yaml", type=str, help="config file in yaml format")
...
def main(args):
    ...
    wandb.init(project=cfg["wandb_project"], config=cfg, 
               name=cfg["exp_name"], id=cfg["exp_name"])
    ...
    if Path(f"exp/{cfg['exp_name']}/stats.npz").exists():
        ...
        train_synth_dataset = SEDDataset(train_synth_df, data_dir=(feat_dir / "train/synthetic"), **kwargs_dataset)
    ...
    ema_model = SEDModel(n_class=len(classes), cnn_kwargs=cfg["model"]["cnn"],
                     encoder_kwargs=cfg["model"]["encoder"],
                     decoder_kwargs=cfg["model"]["decoder"],
                     mask_kwargs=cfg["model"]["mask"]
                     )
