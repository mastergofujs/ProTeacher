...
def parse_args(args):
    ...
    parser.add_argument("--config", default="./config/dcase20_baseline.yaml", type=str, help="config file in yaml format")
...
def main(args):
    ...
    wandb.init(project=wandb_cfg["wandb_project"], config=cfg, 
               name=wandb_cfg["exp_name"], id=wandb_cfg["exp_name"] + '-' + time.strftime("%m-%d-%H:%M"))
